...
| `THRESH` | Min species per orthogroup. With 406 species set to **264** (~65%) |
...
ls "$ORTHO_TSV" "$REF_DNA" "$REF_GFF" "$PANEL" "$DB_JASPAR"
```
...
```
Applies the paper's significance thresholds and combines both database results:
- **E-value < 4e-3** — for counting matched motifs (paper Fig 2b)
...
```
With all fixes applied expect **30–60** of the 256 motifs to match known TFs.
...
| File | Bug | Fix |
|------|-----|-----|
| `01_setup.sh` | Required single-copy orthologs, excluding expanded gene families | Removed single-copy restriction |
| `00_config.sh` | `THRESH=12` (3% coverage) | Set to 264 (65% of 406 species) |
| `encoders.py` | BatchNorm placed after interaction layer, causing motif collapse | Moved BatchNorm to before the interaction Dense layer |
| `07_tomtom.sh` | Only searched JASPAR, missing most Aspergillus TF matches | Now searches JASPAR + Cis-BP 2.0 |
| `parse_tomtom.R` | No significance threshold applied, reporting all 256 hits | Applies E-value < 4e-3 and q ≤ 0.05 thresholds |
| `03_group_by_og.*` | Script was missing entirely | Added as new step between extraction and training |
