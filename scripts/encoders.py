...
def aa_one_hot2(seq,standardize=None,bases = "ACDEFGHIKLMNPQRSTVWY"): ##much faster yay!
	...
	for j in range(0, seq_length):
		if seq[j] in bases: one_hot_seq[j,index[seq[j]]] = 1
	return one_hot_seq
...
class TrainablePooling(tf.keras.layers.Layer):  # learns continuously between max and average pooling
...
class TrainableScaling(tf.keras.layers.Layer):  # scales and subtracts
	...
	def build(self, input_shape):
		self.center = self.add_weight("center", shape=(1, self.fn),
									  initializer=tf.keras.initializers.Constant(self.ic),
									  constraint=tf.keras.constraints.NonNeg(),
									  trainable=self.tr)
		self.scale = self.add_weight("scale", shape=(1, self.fn),
									 initializer=tf.keras.initializers.Constant(self.isc),
									 constraint=tf.keras.constraints.NonNeg(),
									 trainable=self.tr)
		if self.minsc is not None:
			self.min_sc = self.add_weight("min_scale", shape=(1, self.fn),
										  initializer=tf.keras.initializers.Constant(self.minsc),
										  trainable=False)
	...
	def call(self, inputs):
		...
		if self.minsc is not None:
			minS = tf.repeat(self.min_sc, self.sl, axis=0)
			S = tf.math.maximum(S, minS)
		out = S * inputs
		out = out - C
		return out
...
class PWMConstraint(tf.keras.constraints.Constraint):
	## Original Moses lab implementation - do not modify
...
class DNAencoder():
	...
	def interpretable_encoder(self, seq_len, W=10, activation_type='sigmoid'):
		...
		if self.trainable:
			...
			# BatchNorm moved to HERE (before interactions) instead of after.
			# Original had it after: out = weights * matches -> BatchNorm(out)
			# That caused motif collapse. Correct position is before interactions.
			matches = tf.keras.layers.BatchNormalization(center=True, scale=True)(matches)
			# =================================

			weights = tf.keras.layers.Dense(self.filterN,
											use_bias=False,
											activation=None,
